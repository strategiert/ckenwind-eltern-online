-- =====================================================
-- RÜCKENWIND ELTERN - COMPLETE DATABASE SETUP
-- =====================================================
-- Diese Datei enthält alle Migrationen in einer Datei
-- zum einfachen Ausführen im Supabase SQL Editor
-- =====================================================
...
CREATE POLICY "Admins can view all posts" ON public.blog_posts FOR SELECT USING (public.is_admin(auth.uid()));
CREATE POLICY "Admins can insert posts" ON public.blog_posts FOR INSERT WITH CHECK (public.is_admin(auth.uid()));
CREATE POLICY "Admins can update posts" ON public.blog_posts FOR UPDATE USING (public.is_admin(auth.uid()));
CREATE POLICY "Admins can delete posts" ON public.blog_posts FOR DELETE USING (public.is_admin(auth.uid()));

-- Newsletter/Ebook Policies (Service Role)
...
CREATE POLICY "Anyone can update own sessions" ON public.chat_sessions FOR UPDATE USING (user_id IS NULL OR user_id = auth.uid());
...
CREATE POLICY "Anyone can view assessment conditions" ON public.assessment_conditions FOR SELECT USING (true);
CREATE POLICY "Anyone can create assessment conditions" ON public.assessment_conditions FOR INSERT WITH CHECK (true);

-- Glossar Policies
CREATE POLICY "Anyone can view published glossary terms" ON public.glossary_terms FOR SELECT USING (is_published = true);
CREATE POLICY "Admins can view all glossary terms" ON public.glossary_terms FOR SELECT USING (public.is_admin(auth.uid()));
CREATE POLICY "Admins can insert glossary terms" ON public.glossary_terms FOR INSERT WITH CHECK (public.is_admin(auth.uid()));
CREATE POLICY "Admins can update glossary terms" ON public.glossary_terms FOR UPDATE USING (public.is_admin(auth.uid()));
CREATE POLICY "Admins can delete glossary terms" ON public.glossary_terms FOR DELETE USING (public.is_admin(auth.uid()));

CREATE POLICY "Anyone can view sections of published terms" ON public.glossary_sections FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.glossary_terms WHERE id = term_id AND is_published = true));
CREATE POLICY "Admins can manage sections" ON public.glossary_sections FOR ALL USING (public.is_admin(auth.uid()));

CREATE POLICY "Anyone can view references of published terms" ON public.glossary_references FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.glossary_terms WHERE id = term_id AND is_published = true));
CREATE POLICY "Admins can manage references" ON public.glossary_references FOR ALL USING (public.is_admin(auth.uid()));

CREATE POLICY "Anyone can view related terms" ON public.glossary_related_terms FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.glossary_terms WHERE id = term_id AND is_published = true));
CREATE POLICY "Admins can manage related terms" ON public.glossary_related_terms FOR ALL USING (public.is_admin(auth.uid()));

-- Course Policies
CREATE POLICY "Anyone can view published courses" ON public.courses FOR SELECT USING (is_published = true);
CREATE POLICY "Admins can view all courses" ON public.courses FOR SELECT USING (public.is_admin(auth.uid()));
...
CREATE POLICY "Enrolled users can view course lessons" ON public.course_lessons FOR SELECT
...
CREATE POLICY "Users can enroll themselves" ON public.course_enrollments FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update own enrollments" ON public.course_enrollments FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Admins can manage all enrollments" ON public.course_enrollments FOR ALL USING (public.is_admin(auth.uid()));

CREATE POLICY "Users can view own progress" ON public.lesson_progress FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can manage own progress" ON public.lesson_progress FOR INSERT WITH CHECK (user_id = auth.uid());
...
INSERT INTO public.icd10_conditions (code, title, description, category, symptoms, severity) VALUES
